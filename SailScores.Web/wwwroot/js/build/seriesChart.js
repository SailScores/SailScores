...
/// <reference path="./@types/d3-axis/index.d.ts" />
///
import * as d3 from "d3";
import * as d3axis from "d3-axis";
let chartOverallWidth = 960;
let chartOverallHeight = 600;
let margin = 30;
let legendWidth = 150;
let legendLineHeight = 16;
let legendMargin = 2;
d3.json("chartdata2.json").then(processChartData);
function getDate(result, allData) {
    ...
    if (thisRace.length === 1) {
        return new Date(thisRace.date);
    }
...
function responsivefy(svg) {
    ...
    var container = d3.select(svg.node().parentNode), width = parseInt(svg.style("width")), height = parseInt(svg.style("height")), aspect = width / height;
...
function processChartData(data) {
    ...
    function onMouseOver(d) {
        let compId = d.competitorId || d.id;
        svgElement
    ...
    var xAxis = d3axis.axisTop(xScale);
