...
public class CoxSpragueCalculator : BaseScoringCalculator
{
    ...
    protected override void CalculateOverrides(
        SeriesResults resultsWorkInProgress,
        SeriesCompetitorResults compResults)
    {
        foreach (var race in resultsWorkInProgress.SailedRaces)
        ...
        // fix for https://github.com/SailScores/SailScores/issues/413
        // Cox Sprague will treat averages as not scored.
        ...
        foreach (var race in resultsWorkInProgress.Races)
        {
            ...
            if (score != null && IsAverage(score?.RawScore?.Code))
            {
                ...
                // is likely counted as participation.
            }
        }
    }
    ...
    protected override bool IsSeriesBasedScore(ScoreCode scoreCode)
    {
        // I don't know the right answer for this. If a score code is set as
        // average, should it be granted for Cox-Sprague?
        // At time of writing, it seems that no, it shouldn't. If the code is set as
        // participation, it allows for inclusion in the series results.
        // So for now, always returning false seems correct. (for Cox-sprague.)
        return false;
    }
