...
public class SeriesServiceFleetFilteringTests
{
    ...
    [Fact]
    public async Task FleetFiltering_AllBoatsInClubFleetType_IncludesAllCompetitors()
    {
        // Arrange - Create an "All Boats in Club" fleet
        var allBoatsFleet = new Database.Entities.Fleet
        {
            Id = Guid.NewGuid(),
            Name = "All Boats",
        ...
        _context.Fleets.Add(allBoatsFleet);

        // Create competitors - some are explicitly in the fleet, some are not
        var comp1 = new Database.Entities.Competitor
        {
            ...
            CompetitorFleets = new List<Database.Entities.CompetitorFleet>
            {
                new Database.Entities.CompetitorFleet { FleetId = allBoatsFleet.Id }
            }
        };
        var comp2 = new Database.Entities.Competitor
        {
            ...
            ClubId = _clubId,
            BoatClassId = _boatClassId,
            // No CompetitorFleets entry - but should still be included because it's AllBoatsInClub
            CompetitorFleets = new List<Database.Entities.CompetitorFleet>()
        };
        ...
        _context.Competitors.AddRange(comp1, comp2, comp3);

        // Create race with all 3 competitors
        var race1 = new Database.Entities.Race
        {
            Id = Guid.NewGuid(),
            Name = "All Boats Race",
            ...
            Scores = new List<Database.Entities.Score>
            {
                ...
                new Database.Entities.Score { CompetitorId = comp2.Id, Place = 2 },
                new Database.Entities.Score { CompetitorId = comp3.Id, Place = 3 }
            }
        };
        ...
        // Create series filtered to "All Boats" fleet
        var series = new Database.Entities.Series
        {
            Id = Guid.NewGuid(),
            Name = "All Boats Series",
            UrlName = "all-boats-series",
            ClubId = _clubId,
            Season = _season,
            Type = Database.Entities.SeriesType.Standard,
            FleetId = allBoatsFleet.Id,
            UseFullRaceScores = false,
            ScoringSystemId = scoringSystem.Id,
            RaceSeries = new List<Database.Entities.SeriesRace>
            {
        ...
        // Assert - All 3 competitors should be included
        var result = await _service.GetOneSeriesAsync(series.Id);
    ...
    [Fact]
    public async Task FleetFiltering_SelectedClassesFleetType_IncludesCompetitorsByBoatClass()
    {
        // Arrange - Create two boat classes
        var boatClass1 = new Database.Entities.BoatClass
        {
            Id = Guid.NewGuid(),
            Name = "Class 1",
            ClubId = _clubId
        };
        var boatClass2 = new Database.Entities.BoatClass
        {
            Id = Guid.NewGuid(),
            Name = "Class 2",
            ClubId = _clubId
        };
        _context.BoatClasses.AddRange(boatClass1, boatClass2);

        // Create fleet that selects Class 1 only
        var classFleet = new Database.Entities.Fleet
        {
            Id = Guid.NewGuid(),
            Name = "Class 1 Fleet",
            FleetType = Api.Enumerations.FleetType.SelectedClasses,
            ClubId = _clubId,
            FleetBoatClasses = new List<Database.Entities.FleetBoatClass>
            {
                new Database.Entities.FleetBoatClass { BoatClassId = boatClass1.Id }
            }
        };
        _context.Fleets.Add(classFleet);

        // Create competitors - 2 in Class 1, 1 in Class 2
        var comp1Class1 = new Database.Entities.Competitor
        {
            Id = Guid.NewGuid(),
            Name = "Comp 1 Class 1",
            BoatName = "Boat 1",
            ClubId = _clubId,
            BoatClassId = boatClass1.Id,
            CompetitorFleets = new List<Database.Entities.CompetitorFleet>()
        };
        var comp2Class1 = new Database.Entities.Competitor
        {
            Id = Guid.NewGuid(),
            Name = "Comp 2 Class 1",
            BoatName = "Boat 2",
        ...
        var comp1Class2 = new Database.Entities.Competitor
        {
            Id = Guid.NewGuid(),
            Name = "Comp 1 Class 2",
            BoatName = "Boat 3",
            ClubId = _clubId,
            BoatClassId = boatClass2.Id,
            CompetitorFleets = new List<Database.Entities.CompetitorFleet>()
        };

        _context.Competitors.AddRange(comp1Class1, comp2Class1, comp1Class2);

        // Create race with all 3 competitors
        var race1 = new Database.Entities.Race
        {
            Id = Guid.NewGuid(),
            Name = "Mixed Class Race",
            Date = _season.Start.AddDays(1),
            ClubId = _clubId,
            Fleet = classFleet,
            Scores = new List<Database.Entities.Score>
            {
                new Database.Entities.Score { CompetitorId = comp1Class1.Id, Place = 1 },
                new Database.Entities.Score { CompetitorId = comp1Class2.Id, Place = 2 }, // Class 2 - should be excluded
                new Database.Entities.Score { CompetitorId = comp2Class1.Id, Place = 3 }
            }
        };
        ...
        // Create series filtered to Class 1 fleet
        var series = new Database.Entities.Series
        {
            Id = Guid.NewGuid(),
            Name = "Class 1 Series",
            UrlName = "class-1-series",
            ClubId = _clubId,
            Season = _season,
            Type = Database.Entities.SeriesType.Standard,
            FleetId = classFleet.Id,
            UseFullRaceScores = false,
            ScoringSystemId = scoringSystem.Id,
            RaceSeries = new List<Database.Entities.SeriesRace>
            {
        ...
        // Assert - Only Class 1 competitors should be included
        var result = await _service.GetOneSeriesAsync(series.Id);

        Assert.NotNull(result);
        Assert.NotNull(result.FlatResults);
        Assert.Equal(2, result.FlatResults.Competitors.Count());
        Assert.Contains(result.FlatResults.Competitors, c => c.Id == comp1Class1.Id);
        Assert.Contains(result.FlatResults.Competitors, c => c.Id == comp2Class1.Id);
        Assert.DoesNotContain(result.FlatResults.Competitors, c => c.Id == comp1Class2.Id);

        // Scores should be recalculated: comp1Class1 = 1, comp2Class1 = 2 (not 3)
        var comp1Score = result.FlatResults.CalculatedScores.Single(s => s.CompetitorId == comp1Class1.Id);
        var comp2Score = result.FlatResults.CalculatedScores.Single(s => s.CompetitorId == comp2Class1.Id);

        Assert.Equal(1m, comp1Score.TotalScore);
        Assert.Equal(2m, comp2Score.TotalScore);
    }


    [Fact]
    public async Task UpdateSeriesResults_SeriesWithFleetIdAndUseFullRaceScoresFalse_OnlyIncludesFleetCompetitors()
    {
        // Arrange - Setup test data with two fleets and competitors
        ...
        // Create two races - one in fleet1, one in fleet2
        var race1 = new Database.Entities.Race
        {
            Id = Guid.NewGuid(),
            Name = "Race 1",
        ...
        var series = new Database.Entities.Series
        {
            ...
            FleetId = fleet1.Id,
            UseFullRaceScores = false,
            ScoringSystemId = CreateAppendixAScoringSystem().Id,
            RaceSeries = new List<Database.Entities.SeriesRace>
            {
                new Database.Entities.SeriesRace { RaceId = race1.Id },
                new Database.Entities.SeriesRace { RaceId = race2.Id }
        ...
        // Assert - Only Fleet A competitors (comp1, comp2) should be in results, NOT comp3 from Fleet B
        var result = await _service.GetOneSeriesAsync(series.Id);
    ...
    [Fact]
    public async Task UpdateSeriesResults_SeriesWithFleetIdAndUseFullRaceScoresTrue_OnlyIncludesFleetCompetitors()
    {
        // Arrange - Setup test data with two fleets and competitors
        var clubId = _context.Clubs.First().Id;
        var season = _context.Seasons.First();
    ...
    [Fact]
    public async Task UpdateSeriesResults_SeriesWithFleetIdAndUseFullRaceScoresNull_OnlyIncludesFleetCompetitors()
    {
        // Arrange - Setup test data with two fleets and competitors
        var clubId = _context.Clubs.First().Id;
        var season = _context.Seasons.First();
        ...
        // Create series with FleetId set to fleet1 and UseFullRaceScores = null
        var series = new Database.Entities.Series
        {
            Id = Guid.NewGuid(),
            Name = "Fleet A Series",
    ...
    [Fact]
    public async Task UpdateSeriesResults_SeriesWithFleetIdAndFirstRaceNotInFleet_OnlyIncludesFleetCompetitors()
    {
        // Arrange - Setup test data with two fleets and competitors
        var clubId = _context.Clubs.First().Id;
        var season = _context.Seasons.First();
        ...
        // Create series with FleetId set to fleet2 and UseFullRaceScores = null
        var series = new Database.Entities.Series
        {
            Id = Guid.NewGuid(),
            Name = "Fleet A Series",
        ...
        // Assert - Only Fleet 2 competitors (comp3) should be in results, NOT comps from Fleet 1
        var result = await _service.GetOneSeriesAsync(series.Id);
    ...
    public async Task UpdateSeriesResults_SeriesWithFleetIdAndDNCScores_DNCEquals2()
    {
        ...
        // FlatResult for the competitor with a DNC should be 3
        Assert.Equal(3, result.FlatResults.CalculatedScores.Single(s => s.CompetitorId == comp2.Id).TotalScore);
        Assert.DoesNotContain(result.FlatResults.Competitors, c => c.Id == comp3.Id);
    }
